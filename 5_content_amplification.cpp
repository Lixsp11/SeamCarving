...
int main(int argc, char *argv[]) {
    ...
    namedWindow("Content Amplification");

    double fx = atoi(argv[3]) * 1.0 / img.cols, fy = atoi(argv[4]) * 1.0 / img.rows;
    // Choose the bigger side resize
    if(fy > fx) {
        resize(img, img, Size(), fy, fy, fy < 1 ? INTER_AREA : INTER_LINEAR);
        stack<Point> seam;
        // Seam carving the other side.
        while (img.cols > atoi(argv[3])) {
            ...
            imshow("Content Amplification", img);
            waitKey(10); // Sleep 0.01s to show the process of seam carving.
        }
    }
    else {
        resize(img, img, Size(), fx, fx, fx < 1 ? INTER_AREA : INTER_LINEAR);
        stack<Point> seam;
